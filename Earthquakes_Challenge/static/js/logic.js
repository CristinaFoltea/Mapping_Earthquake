...
let tectonicPlatesOverlay = new L.layerGroup();
...
d3.json(earthquakeLink).then(function(data) {
  ...
  legend.onAdd = function() {
    ...
    for (var i = 0; i < magnitudes.length; i++) {
      div.innerHTML += `<div><i style="background:${colors[i]}"></i> ${magnitudes[i]} ${magnitudes[i + 1] ? "–" + magnitudes[i + 1] + "" : "+"}</div>` 
   }
    return div;
  };
